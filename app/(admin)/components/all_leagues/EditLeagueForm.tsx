...
import { editLeagueSchema } from "@/lib/validation/editLeagueSchema";
...
export default function EditLeagueForm({
  ...
  const validateForm = () => {
    const result = editLeagueSchema.safeParse(form);
    if (!result.success) {
  ...
  const handleChange = <K extends keyof typeof form>(
    ...
    setForm((prev) => {
      ...
      const result = editLeagueSchema.safeParse(updatedForm);
      if (!result.success) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_DOMAIN_URL}/api/admin/edit_league/${league.id}`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
      ...
      if (res.ok || data.status === "success") {
        Toast.fire({
          icon: "success",
          title: data.message || "League updated successfully!",
        });
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 w-full">
        ...
        <DropzoneUploader
          ...
          initialUrl={league.backgroundImage || ""}
        />
      </div>
